...
const Events = () => {
  ...
  // Función para obtener los eventos de una fecha específica, considerando eventos de varios días
  ...
  const upcomingEvents = events.filter(event => new Date(event.endDate) >= currentDate);
  const pastEvents = events.filter(event => new Date(event.endDate) < currentDate);

  return (
    <div className="p-8 grid grid-cols-4 gap-4">
      {/* Sección de 3/4 para los eventos */}
      ...
      {/* Sección de 1/4 para el calendario y los eventos programados */}
